...
The probabilities are then computed through softmax which incorporates the mutual exclusivity between the 3 segmentation masks.
\subsection{Training}
...
of the noise effects on the images are provided in the Results section. A visual representation of noise is provided in appendix (\ref{chap:Pert})
\vskip1em
...
This analysis have been performed for both physician result, also a correlation between the two physician have been processed in order to compare with the Dice result.
The both physicians have evaluated in the same conditions to avoid any bias.
\vskip1em
