...
The following graphs illustrate the impact of perturbations on segmentation performance. 
...
various segmentation metrics (\ref{list:metrics}), which indicate the extent of variation in segmentation performance due to the perturbations. 
...
\vskip1em
The graphs below represent the scenarios in which perturbations are applied to the nodal and primary tumor labels in the CT modality.
\begin{figure}[ht]
    \centering
...
The graphs illustrate significant variations in the Dice coefficient under spike, bias, and noise perturbations, 
...
The remaining three categories (Primary in CT, Nodal and Primary in PET) are provided in the appendix (\ref{chap:CorrProp}). A more comprehensive correlation graph, encompassing all 
...
\vskip1em
Interestingly, ghost perturbation reveals a prominent negative correlation, suggesting an improvement in segmentation performance as the property values increase, with the strongest correlation observed with the compactness property.
\begin{figure}[ht]
    \centering
...
\vskip1em
For intensity variations in CT scans, only noise demonstrated a significant correlation, with coefficients of 0.38 and 0.31 at the first two degrees of severity. The scatter plots behavior remained consistent, though the outliers were observed around variations of 20.
...
\begin{figure}[ht]
    ...
    \label{fig:three_subfigures}
\end{figure}
For PET variations, the correlation with noise was lower; however, spike perturbations demonstrated stronger correlations, reaching up to 0.41. The scatter distributions were similar to those for CT, with outliers observed around variations of 0.5.
\vskip1em
In terms of contrast, PET exhibited higher correlations, particularly with spike, which reached a maximum of 0.61. Noise showed a correlation of 0.38 at the third degree of severity, while bias peaked at a moderate correlation of 0.29. The scatter plots for both modalities showed similar distributions. However, there were noticeable changes in property values where the outliers appeared. The baseline distribution for contrast in PET was more aligned with a diagonal when compared to CT.\newpage
\begin{figure}[ht]
    \centering
...
For the last four properties analyzed, the SUVmax demonstrated a moderate correlation with spike perturbations, ranging from 0.29 to 0.43 across the degrees of severity. Noise and bias also exhibited correlations, although at more moderate levels, around 0.20. 
\vskip1em
In the case of CT number, spike perturbations again displayed the strongest correlation, peaking at 0.32, while a unexpected correlation of 0.28 was observed with ghost perturbations at the third degree of severity. Notably, noise exhibited irregular behavior, with correlation coefficients fluctuating between -0.40 and 0.20.
\vskip1em
The regions showed varied behavior across properties, except for spike, which revealed a stable negative correlation ranging from -0.16 to -0.26. 
\vskip1em
...
\vskip1em
In terms of scatter plot behavior, no significant new patterns emerged. However, entropy displayed a distinct crescent-shaped distribution, consistent with the trends observed for the first three properties analyzed previously.
\vskip1em
...
\vskip1em
Spike emerges as the perturbation with the highest correlation scores, followed by noise and then bias. In contrast, ghost perturbations generally show an improvement in segmentation performance as most property values decrease, with the strongest negative correlation observed for compactness. The only exception is CT contrast, which displays a positive correlation with ghost perturbations.
\vskip1em
...
Furthermore, across all degrees of severity for each perturbation, the correlation behavior is mostly nonlinear with increasing severity levels.
...
In contrast, the perturbation analysis indicates that the nodal channel was significantly more affected by the applied perturbations in the selected cases with majority of 1 and 2. Conversely, the distribution of evaluations for the primary channel remains consistent with that of the original cases but with some changes, including surprisingly more evaluations of 4 stars and none for 5.
\vskip1em
Comparing with the second observer, for the original cases, the distribution for the nodal channel is more equal across 2, 3 and 4 stars with each category around 30\%, and with 2 and 3 stars being more prevalent.
However, for the primary tumors, the 4-stars percentage is higher than before with also a 1-star more prevalent. 
For the perturbed cases, the same behavior observed in nodal label with even worse impact. For the primary, the only major change is that 3-stars are a bit more present at expense of the 4-stars.
\vskip1em
...
Which would arbitrarily represent the gradings in Likert scale.
\begin{figure}[ht]
    \centering
