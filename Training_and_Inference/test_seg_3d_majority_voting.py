...
class MedicalSlicesDataset(Dataset):
   ...
   """
    ...
    def __getitem__(self, idx):
        ...
        #Reorganizing the dimension
        img = np.transpose(img, (0, 3, 1, 2))
        seg = np.transpose(seg, (0, 3, 1, 2))

        # Prepare segmentation masks (multiple binary)
...
def perform_inference_and_save(models, perturbation, output_dir, device):
    """
        Perform inference using multiple models and save the segmentation results, while calculating Dice scores.
...
#The Baseline and the 6 perturbations with 3 degrees of severity
perturbations = [
    ('Baseline', None),
